...
async function loadCategoryTracks() {
    try {
        ...
        const { data: tracks, error } = await supabaseClient
            ...
            .select(`
                *,
                categories:category_id(id, name, body, image_url, slug),
                track_tags(
                    tags(id, name, color)
                )
            `)
        ...
        if (!categoryName && tracks[0].categories) {
            document.getElementById('categoryName').textContent = tracks[0].categories.name;
        }

        // Show category description if exists
        if (tracks[0].categories.body) {
            document.getElementById('categoryDescription').textContent = tracks[0].categories.body;
        }
        
        // Show category image if exists
        if (tracks[0].categories.image_url) {
            ...
            imageElement.src = tracks[0].categories.image_url;
            imageElement.alt = tracks[0].categories.name;
